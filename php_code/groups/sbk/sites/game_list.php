...
$arr_matches = DBMapper::get_list_Spieltag(0);
for($i=0; $i<count($arr_matches[0]); $i++) {
  for($k=0; $k<count($arr_league[0]); $k++) {
    if($arr_matches[2][$i] == $arr_league[0][$k]) {
      $arr_matches[1][$i] .= " ".$arr_league[1][$k];
      break;
    }
  }
  /*
...
if($bool_show_data) {
  ...
  if($int_id_matchday) {
    ...
    for($i=0; $i<count($arr_games[0]); $i++) {
      ...
      if ($Team1) {
        $name_team1 = $Team1->get_str_name();
      } else {
        $name_team1 = "NA";
      }
      if ($Team2) {
        $name_team2 = $Team2->get_str_name();
      } else {
        $name_team2 = "NA";
      }
      $str_text = $arr_games[3][$i]." ".$name_team1." : ".$name_team2;
      if($i%2) {
